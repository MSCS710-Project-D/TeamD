...
async function createMessage(req, res) {
    try {
        const { sender, receiver, content, role } = req.body;
        const admin = await User.findOne({username: 'sp@gmail.com'});

        const config = {
            sender: role === 'admin' ? admin._id : sender,
            receiver: role !== 'admin' ? admin._id : receiver,
            content,
        }

        const newMessage = new Message(config);

        const savedMessage = await newMessage.save();

        if (role === 'admin') {
            pusher.trigger('chatbot', receiver, {
                message: savedMessage
            })
        } else {
            pusher.trigger('chatbot', 'admin', {
                message: savedMessage
            })
        }
